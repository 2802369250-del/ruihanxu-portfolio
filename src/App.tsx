  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-white p-8">
      <header className="max-w-4xl mx-auto mb-12">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Ruihan Xu</h1>
        <p className="text-neutral-400">Portfolio · 作品集</p>
      </header>
      
      <main className="max-w-4xl mx-auto">
        {PROJECTS.map((project) => (
          <div key={project.id} className="border border-neutral-800 rounded-2xl p-6 bg-neutral-950/50 backdrop-blur">
            ...
            {project.chapters?.map((chapter, index) => (
              <div key={index} className="mt-6 border-t border-neutral-800 pt-6">
                <h3 className="text-sm font-mono text-neutral-500">{chapter.chapter} · {chapter.title}</h3>
                <h4 className="text-lg font-medium mt-2 whitespace-pre-line text-neutral-200">{chapter.subtitle}</h4>
                <p className="text-sm text-neutral-400 mt-2 whitespace-pre-line leading-relaxed">{chapter.content}</p>
              </div>
            ))}
            
            <div className="mt-8 border-t border-neutral-800 pt-6">
              <h3 className="text-sm font-mono text-neutral-500 mb-4">DETAILED MANIFESTO</h3>
              <p className="text-sm text-neutral-400 whitespace-pre-line leading-relaxed">{project.detail}</p>
            </div>
          </div>
        ))}
      </main>
